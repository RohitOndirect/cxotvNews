...
const Edtech = () => {
    ...
    const fetchCXOTalkData = async (page = 1) => {
        try {
            ...
            const response = await fetch(
                `https://apicxotv.techplusmedia.com/api/news?populate=*&sort=publishedAt:DESC&filters[$or][0][categories].data[name][$eq]=EdTech&filters[$or][1][subcategories].data[name][$eq]=EdTech&pagination[page]=1`
            );
    ...
    return (
        <div className="flex flex-col w-full h-full">
            <HeaderAdjust logo={EducationLogo} />
